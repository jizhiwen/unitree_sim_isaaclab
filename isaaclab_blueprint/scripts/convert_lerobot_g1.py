...
class DatasetFormatter:
    ...
    def write_metadata(
        ...
        if len(STATE_MODALITIES) == 0:
            _state_modalities["single_arm"] = {"start": 0, "end": self.state_dim}
        else:
            _state_modalities = STATE_MODALITIES

        _action_modalities = {}
        if len(ACTION_MODALITIES) == 0:
            _action_modalities["single_arm"] = {"start": 0, "end": self.action_dim}
        else:
    ...
    with h5py.File(dataset_file, "r") as f:
        ...
        for dataset in range(0, min(episode_num, len(f["data"]))):
            ...
            for row in range(0, deta_actions.shape[0]):
                ...
                cur_action = np.zeros(5, dtype=np.float32)
                cur_action[:] = joint_action
                # cur_action[-1] = 1000 if step[-1] == 1 else 0
                actions.append(copy.deepcopy(cur_action))
            actions = np.array(actions, dtype=np.float32)

            states[:, :] = np.degrees(states[:, :])
            actions[:, :] = np.degrees(actions[:, :])
            ...
            for row in range(min(states.shape[0], actions.shape[0])):
                ...
                step_data = {
                    ...
                    "observation.state": [
                        states[row][G1_JOINT_NAMES.index(name)]
            ...
            start_frame, end_frame = trial_nums[episode_index + dropped_episode_num]
            ...
            if "rgb" in args_cli.type:
                ...
                for frame_idx in range(video_start + 1, video_start + video_length):
                    file_path_rgb = os.path.join(
                        image_path,
                        frame_name_pattern.format(trial_num=episode_index + dropped_episode_num, frame_idx=frame_idx),
                    )
                ...
                file_path_rgb = os.path.join(
                    image_path,
                    frame_name_pattern.format(
                        trial_num=episode_index + 1 + dropped_episode_num, frame_idx=video_start + video_length
                    ),
                )
                pil_image = Image.open(file_path_rgb)
                episode_frames_pil.append(pil_image)
            ...
            if "semantic_segmentation" in args_cli.type:
                ...
                for frame_idx in range(video_start + 1, video_start + video_length):
                    file_path = os.path.join(
                        image_path,
                        segmentation_name_pattern.format(
                            trial_num=episode_index + dropped_episode_num, frame_idx=frame_idx
                        ),
                    )
            ...
            if "depth" in args_cli.type:
                ...
                for frame_idx in range(video_start + 1, video_start + video_length):
                    file_path = os.path.join(
                        image_path,
                        depth_name_pattern.format(trial_num=episode_index + dropped_episode_num, frame_idx=frame_idx),
                    )
                    pil_image = Image.open(file_path)
                    episode_frames_pil.append(pil_image)
                file_path = os.path.join(
                    image_path,
                    depth_name_pattern.format(
                        trial_num=episode_index + 1 + dropped_episode_num, frame_idx=video_start + video_length
                    ),
                )
                pil_image = Image.open(file_path)
